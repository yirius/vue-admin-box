  ...
  setup () {
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({ page, sorts, filters, form }) => {
            ...
            return getRequest(
                `/restful/thinker/monitor/systemLogs?limit=${page.pageSize}&page=${page.currentPage}&${XEUtils.serialize(queryParams)}`,
                {}
            ).then(response => response.data);
          },
          // 当点击工具栏删除按钮或者手动提交指令 delete 时会被触发
          delete: ({ body }) => {
            return fetch(`/restful/thinker/monitor/systemLogs`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(response => response.json())
          },
      ...
      columns: [
        ...
        { field: 'content', title: 'Nickname', showOverflow: true, editRender: { name: 'input', attrs: { placeholder: '请输入昵称' } }, formatter: contentFormatter },
        {
          field: 'sex',
          title: 'Sex',
          filters: [
      ...
      importConfig: {
        ...
        importMethod ({ file }) {
          ...
          return fetch(`/api/pub/import`, { method: 'POST', body: formBody }).then(response => response.json()).then(data => {
