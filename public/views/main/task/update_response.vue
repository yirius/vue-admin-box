  ...
  <StepLayer :layer="layer">
    ...
    <el-row>
      <el-table style="margin-left: 15px" stripe v-if="failed.length!=0" :data="failed" style="width: 100%">
        <el-table-column width="150" label="广告主ID" prop="advertiser_id"/>
        <el-table-column prop="ad_id" label="广告ID" width="150" :show-overflow-tooltip='true'/>
...
export default {
  ...
  watch: {
    layer: {
      ...
      handler(newV, oldV) {
        if (newV.show) {
          console.log(this.layer.responseResult)
          if(this.layer.responseResult&&this.layer.responseResult) {
